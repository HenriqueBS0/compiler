...
class Concatenacao extends Node {
    ...
    private function validarVariavel(AnalisadorSemantico $analisadorSemantico, Token $variavel) : void
    {
        ...
        $this->verificarVariavelTipoCadeia($analisadorSemantico, $variavel);
    ...
    private function verificarVariavelTipoCadeia(AnalisadorSemantico $analisadorSemantico, Token $variavel) : void
    {
        $nomeVariavel = $variavel->getLexeme();

        if($analisadorSemantico->getVariaveis()->getVariavel($nomeVariavel)->getTipo() !== 'STR') {
            $linha = $variavel->getPosition()->getStartLine();
            $analisadorSemantico->newSemanticException("Erro na linha {$linha}: A variável '{$nomeVariavel}' não é do tipo 'STR'.");
        }
    }
